...
def load_data(data_folder_name):
    ...
    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'voronoi_foams', 'results', f"{data_folder_name}", 'beams.pkl'), 'rb') as f:
        beams = pickle.load(f)

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'voronoi_foams', 'results', f"{data_folder_name}", 'seeds.pkl'), 'rb') as f:
        seeds = pickle.load(f)
...
def test_annulus():
    ...
    diag = simplex_tree.persistence(min_persistence=.1)
